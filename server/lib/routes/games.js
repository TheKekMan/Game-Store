...
Router.get("/:id", async (req, res) => {
  try {
    ...
    const dev = await pool.query("SELECT dev_name FROM dev_profile WHERE dev_profile_id = $1", [game.rows[0].developer]);
    if (dev.rowCount == 0) {
...
Router.get("/search/:query", async (req, res) => {
  try {
    ...
    const games = await pool.query("SELECT * FROM games WHERE (game_name ilike $1 OR game_name ilike $2 OR game_name ilike $3) OR (genre ilike $1 OR genre ilike $2 OR genre ilike $3)", [searchExp1, searchExp2, searchExp3]);
    res.json({ gameResults: games });
  } catch (err) {
    console.error(err.message);
    if (err.kind === "ObjectId") {
...
Router.get("/tag/:query", async (req, res) => {
  try {
    ...
    const games = await pool.query("SELECT * FROM games WHERE genre ilike $1", [searchTag]);
    res.json({ gameResults: games });
  } catch (err) {
    console.error(err.message);
    if (err.kind === "ObjectId") {
...
Router.get("/dev/:query", async (req, res) => {
  try {
    ...
    const games = await pool.query("SELECT * FROM games WHERE developer = $1", [searchId]);
    res.json({ gameResults: games });
  } catch (err) {
    console.error(err.message);
    if (err.kind === "ObjectId") {
